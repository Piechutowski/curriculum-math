# Exam To-Do — personal tick-list

**This file is deliberately outside the tracking system.** The progress tool only reads the
seven registered module files, so boxes ticked here change no bars, no counts, no log. Real
lesson progress still gets ticked in modules 01/02/03 as always — this page is the exam plan
([EXAM-PLAN.md](EXAM-PLAN.md)) turned into boxes, one per lesson, for the satisfaction of
crossing each one off. Tick freely.
